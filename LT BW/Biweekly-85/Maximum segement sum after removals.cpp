//video-link : https://www.youtube.com/watch?v=L_EU1nxzFKg&t=873s
 ...
 #define ll long long
    vector<long long> maximumSegmentSum(vector<int>& nums, vector<int>& remove) {
        ...
        for(int i = 0; i < n; i++){
            ...
            ll cur_sum = pre[r-1] - ( l == -1 ? 0 : pre[l] ); //pre[r-1] - pre[l] gives sum of l+1 to r-1 //segment
            mp.erase(mp.find(cur_sum));                 // remove current segment sum

            ll left_sum = ( cur == 0 ? 0 : pre[cur-1] ) - (l == -1? 0 : pre[l] ); //sum from l+! to cur-1
            ll right_sum = (r == 0 ? 0 : pre[r-1]) - pre[cur]; //sum from cur+1 to r-1
